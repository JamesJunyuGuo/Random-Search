...
class Sampler(object):
    ...
    def sample_with_learner_using_angle(self, num_directions, gpu=True):
        ...
        # print("Values = ", values)
        # print("value sum  is ", np.sum(values))
    ...
    def sample_with_learner_using_jacobian(self, normalize, gpu=True):
        if self.learner.filter_corrected:
            # print("Filtered corrected")
            if normalize:
                
                J = self.learner.get_jacobian_corrected(self.current_solution, normalize=True, gpu=gpu)
