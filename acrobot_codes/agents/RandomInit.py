...
class RandomInit:
    ...
    def step(self, noise: np.array, crpo_step = 10, crpo_episodes = 10, cg_iters = 10, limit_1 = 50, limit_2 = 50, direction = 0):
        ...
        ## Initial neural network
        self.policy = DQNSoftmax(self.input_size, self.output_size)
        self.value_function = ValueFunctionWrapper(DQNRegressor(self.input_size), self.value_function_lr)
        self.cost_value_function_1 = ValueFunctionWrapper(DQNRegressor(self.input_size), self.value_function_lr)
        self.cost_value_function_2 = ValueFunctionWrapper(DQNRegressor(self.input_size), self.value_function_lr)
        ## Cuda identity
        # self.policy.cuda()
        # self.value_function.cuda()
        # self.cost_value_function_1.cuda()
