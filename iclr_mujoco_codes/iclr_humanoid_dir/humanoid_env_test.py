...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        ctrl_cost = self.control_cost(action) * 1.0

        forward_reward = self._forward_reward_weight * (y_velocity*np.sin(self._pi) + x_velocity*np.cos(self._pi))
        # x_velocity, y_velocity = x_velocity/np.sqrt(x_velocity**2 + y_velocity**2), y_velocity/np.sqrt(x_velocity**2 + y_velocity**2)
        # forward_reward = np.square(x_velocity - np.cos(self._pi)) + np.square(y_velocity - np.sin(self._pi))
        ...
        # cost = -(np.abs(observation[18] + 1.0) + np.abs(observation[21] + 1.0))

        cost = -(np.abs(observation[16] - 0.75) + np.abs(observation[19] + 0.75))
        cost += -(np.abs(observation[17] + 0.75) + np.abs(observation[20] - 0.75))
        cost += -(np.abs(observation[18] + 1.5) + np.abs(observation[21] + 1.5))
        cost += -(np.abs(observation[5]))
        cost += -(np.abs(observation[6]))*4 # test good posture

        info = {
            "reward_linvel": forward_reward,
            # "reward_linvel": -np.square(x_velocity - np.cos(self._pi)) - np.square(y_velocity - np.sin(self._pi)),
